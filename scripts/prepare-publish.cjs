...
// 复制 scripts/postinstall.cjs
const postinstallSource = path.join(root, 'scripts', 'postinstall.cjs')
const postinstallTarget = path.join(scriptsDir, 'postinstall.cjs')
if (fs.existsSync(postinstallSource)) {
  fs.copyFileSync(postinstallSource, postinstallTarget)
  console.log('[pb2ts] ✓ postinstall.cjs copied')
}
